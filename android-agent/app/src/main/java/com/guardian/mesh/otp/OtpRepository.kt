...
object OtpRepository {
    ...
    fun getLatestOtp(): OtpEntry? {
        // Return whatever we have, let consumer decide on expiration if needed, or enforce same logic
        val otp = currentOtp ?: return null
        val age = System.currentTimeMillis() - otp.timestamp
        return if (age < EXPIRATION_MS) otp else null
    }
